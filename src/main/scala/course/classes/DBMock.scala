...
object DBMock {
  ...
  def getUserById(id: Int): Future[Option[User]] = Future.failed(new Exception("fails"))
  def getAdressByUser(user: User): Future[Option[Address]] = Future.successful(Some(Address("trans","rionegro")))

  def getAddressById(id: Int): OptionT[Future, Address] = for{
