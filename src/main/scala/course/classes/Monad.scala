...
object monadValidations{
  def leftIdentity[F[_],A](x: A)(f: A => F[A])(implicit m: Monad[F]): Boolean = m.flatMap(m.pure(x))(f) == f(x)
  def rightIdentity[F[_],A](x: A)(implicit m: Monad[F]) = m.flatMap(m.pure(x))(m.pure) == m.pure(x)
  def associativity[F[_],A](x: A)(f: A => F[A])(g: A => F[A])(implicit m: Monad[F]) = m.flatMap(m.pure(x))(_ => m.flatMap(f(x))(g)) == m.flatMap(m.flatMap(m.pure(x))(f))(g)
}
object monadOps {
