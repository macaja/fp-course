...
object Collection {
  ...
  def length[A](col: Collection[A]): Int = foldRight(col, 0) { (_, acc) =>
    acc + 1
  }
}
...
object CollectionInt {
  ...
  def productFold(col: Collection[Int]) = Collection.foldRight(col, 1) {
    (acc, v) =>
      acc * v
  }
  ...
  def maxFold(col: Collection[Int]) =
    Collection.foldRight(col, Integer.MIN_VALUE)(_ max _)
